...
| `durable` | `false` | Session-only — dies when Claude Code exits. Each new session starts its own timer. |
...
- **Session-only by default.** The timer disappears when Claude Code exits. Each new session should set up its own timer (the wizard handles this).
...
> "Claude Code rate-limits based on a rolling 5-hour window. This sets up a lightweight background ping every 3 hours that keeps that window rolling smoothly, so you get more usage out of long sessions before being rate-limited. It runs automatically while Claude Code is open, goes away when you close it, and auto-expires after 7 days."
